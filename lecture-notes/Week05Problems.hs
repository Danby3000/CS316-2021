...
instance Semigroup (MaybeFun a) where
  MkMaybeFun f <> MkMaybeFun g = MkMaybeFun (\x -> case g x of 
    Nothing -> Nothing 
    Just x' -> f x')

instance Monoid (MaybeFun a) where
  mempty = MkMaybeFun Just
...
-- fold' :: (Monoid a, Foldable c) => c a -> a
-- fold' = foldList . Week05Problems.toList
--   where
--    foldList [] = mempty 
...
roughlyHowTrue = foldMap (\a -> if a then One else Zero)
...
productAll xs = getProduct(foldMap Product xs)
